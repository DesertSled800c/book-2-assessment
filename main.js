...
*/
const calculateYearlyExpenses = (app) => {
    const annualExpenses = app.monthlyExpenses * 12
    return annualExpenses
 }

const calculateExpensesPercentage = (app, yearlyExpenses) => {
    const expensePercent = yearlyExpenses / app.salary * 100
    return expensePercent
 }

const isQualified = (app, expensePercent) => {
    if (expensePercent < 10) {
        app.mortgage.qualified = true
        app.mortgage.amount = app.salary * 5
    } else if (expensePercent >= 10) {
        app.mortgage.qualified = false
        app.mortgage.amount = 0
    }
    return app
 }
...
/*
    ...
    */
    let message = "";
    for (const app of mortgageApplicants) {
    let yearlyExpenses = calculateYearlyExpenses(app)
    let expensePercent = calculateExpensesPercentage(app, yearlyExpenses)
    let qualified = isQualified(app, expensePercent) 
    if (app.mortgage.qualified === true) {
        message += `${app.name} is qualified for a maximum mortgage of $${app.mortgage.amount}\n`
    }
    // return message
} 
